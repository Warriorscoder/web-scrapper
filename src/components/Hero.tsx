...
function Hero() {
  ...
  const [dailyRequestsLeft, setDailyRequestsLeft] = useState<number | null>(
    null
  );
  ...
  const fetchRequestStatus = async () => {
    try {
      const response = await axios.get(
        `${window.location.origin}/api/rate-limit-status`
      );
  ...
  const handleSubmit = async () => {
    ...
    //     structuredData = response.data; // Direct array from API
    //   } else if (Array.isArray(response.data.structuredData)) {
    //     structuredData = response.data.structuredData.flat();
    //   }
  ...
  const isInputDisabled =
    isLoading || (dailyRequestsLeft !== null && dailyRequestsLeft <= 0);
  const tableHeaders = scrapeResult?.structuredData?.[0]
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center justify-center font-sans overflow-hidden p-4">
      ...
      <div className="absolute top-4 right-4 z-20 bg-gray-900/50 backdrop-blur-sm border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-gray-300">
        Requests Left Today:{" "}
        {dailyRequestsLeft === null ? "..." : dailyRequestsLeft}
      </div>
          ...
          <div className="w-full bg-gray-900/70 border border-gray-700 rounded-2xl p-6 text-left animate-fade-in-up">
                ...
                <tbody>
                  ...
                  scrapeResult.structuredData.length > 0 ? (
                    scrapeResult.structuredData.map((row, index) => (
                      <tr
                        key={index}
                        className="bg-gray-900/50 border-b border-gray-700"
                      >
                        {tableHeaders.map((header) => (
                          <td key={header} className="py-4 px-6 break-words">
                            {String(
                              (row as Record<string, unknown>)[header] ?? "N/A"
                            )}
                          </td>
                        ))}
                      </tr>
            ...
            <div className="mt-6 border-t border-gray-700 pt-4 flex justify-end gap-4">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-semibold text-gray-900 bg-cyan-500 rounded-md hover:bg-cyan-400 transition-colors"
              >
                Download Excel
              </button>
            </div>
            ...
            <div className="h-8 mb-2">
              {dailyRequestsLeft !== null && dailyRequestsLeft <= 0 && (
                <div className="text-yellow-400 animate-fade-in-up">
                  Today&apos;s limit has been reached. Please come back
                  tomorrow.
                </div>
              )}
            </div>
